...
def _load_jit_sources():
    _check_cute_runtime_available()
    try:
        from .src.fmha import fmha as jit_fmha
        from .src.helpers import fmha_helpers
    except (ImportError, OSError) as exc:
        raise ImportError(f"CuTe DSL FMHA JIT sources are not available: {exc}") from exc
    return jit_fmha, fmha_helpers
...
def _get_jit_mask_type(
    fmha_helpers,
    is_causal: bool,
    window_left: int,
    window_right: int,
):
    if is_causal or window_left != -1 or window_right != -1:
        return fmha_helpers.MaskEnum.WINDOW_MASK
    return fmha_helpers.MaskEnum.RESIDUAL_MASK


def _get_jit_fmha_kernel(
    q_4d: torch.Tensor,
    k_4d: torch.Tensor,
    v_4d: torch.Tensor,
    o_4d: torch.Tensor,
    problem_size: tuple[int, int, int, int, int, int, int, int],
    qo_indptr: Optional[torch.Tensor],
    kv_indptr: Optional[torch.Tensor],
    lse: Optional[torch.Tensor],
    is_causal: bool,
    ...
    window_left: int,
    window_right: int,
    ws_left: Optional[Any],
    ws_right: Optional[Any],
    stream: Optional[Any],
):
    jit_fmha, fmha_helpers = _load_jit_sources()
    _, _, _, _, _, _, head_dim, value_head_dim = problem_size
    if head_dim != value_head_dim:
        raise ValueError("CuTe DSL FMHA JIT requires Q/K and V to use the same head dimension.")
    mask_type = _get_jit_mask_type(
        fmha_helpers,
        is_causal,
        window_left,
        window_right,
    ...
    if compile_key in _JIT_COMPILE_CACHE:
        return _JIT_COMPILE_CACHE[compile_key]
    ...
    if stream is None:
        stream = cuda_driver.CUstream(torch.cuda.current_stream(q_4d.device).cuda_stream)
    fmha = jit_fmha.BlackwellFusedMultiHeadAttentionForward(
        jit_fmha.Float32,
        jit_fmha.Float32,
        (128, 128, head_dim),
        is_persistent,
    ...
    _JIT_COMPILE_CACHE[compile_key] = cute.compile(
        fmha,
        q_cute.iterator,
        k_cute.iterator,
        v_cute.iterator,
...
def launch_cute_dsl_jit_fmha(
    ...
    _, _, _, _, _, _, head_dim, _ = problem_size
    if head_dim not in (32, 64, 128):
        raise ValueError("CuTe DSL FMHA JIT supports head dimensions 32, 64, and 128.")
    if q_4d.dtype != k_4d.dtype or q_4d.dtype != v_4d.dtype:
        raise ValueError(
            "CuTe DSL FMHA JIT requires matching Q/K/V dtypes. "
            "Packaged cubins are required for mixed QK/PV dtype variants."
        )
    ...
    if stream is None:
        stream = cuda_driver.CUstream(torch.cuda.current_stream(q_4d).cuda_stream)
    ...
    kernel_fn = _get_jit_fmha_kernel(
        q_4d,
        k_4d,
        v_4d,
        o_4d,
...
def cute_dsl_fmha_fwd(
    ...
    if kernel_fn is None and allow_cubins:
        ...
        except (AttributeError, FileNotFoundError, ImportError, ValueError):
            kernel_fn = None
    ...
    # JIT path: when CUBIN is not found

    if kernel_fn is None:
        if use_skip_softmax:
            raise ValueError("CuTe DSL FMHA skip-softmax requires a packaged cubin kernel.")
        launch_cute_dsl_jit_fmha(
            q_4d,
            k_4d,
            v_4d,
            o_4d,
