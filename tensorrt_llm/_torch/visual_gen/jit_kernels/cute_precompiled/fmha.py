...
def _get_cute_dsl_fmha_kernel_cached(
    variant_name: str,
    gpu_arch: str | None,
    enable_tvm_ffi: bool,
):
    path = _resolve_cubin_path(variant_name, gpu_arch)
    return _load_kernel_from_path(path, variant_name, enable_tvm_ffi)
...
def get_cute_dsl_fmha_kernel(
    ...
    return _get_cute_dsl_fmha_kernel_cached(
        variant_name,
        gpu_arch,
        enable_tvm_ffi,
    )
...
def cute_dsl_fmha_context_forward(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    o: torch.Tensor,
    ...
    if enable_tvm_ffi:
        kernel_fn(
            ...
            o_4d.data_ptr(),
            problem_size,
            qo_indptr.to(torch.int32) if varlen else None,
            kv_indptr.to(torch.int32) if varlen else None,
    ...
    float8_e4m3fn = getattr(torch, "float8_e4m3fn", None)
    is_fp8_qk = q.dtype == float8_e4m3fn
    is_fp8_pv = v.dtype == float8_e4m3fn
    is_fp8_out = o.dtype == float8_e4m3fn
    if is_fp8_qk:
        q_cute = from_dlpack(q_4d.view(torch.int8), assumed_align=16)
        q_cute = q_cute.mark_layout_dynamic(leading_dim=3)
        q_cute.element_type = cutlass.Float8E4M3FN
        k_cute = from_dlpack(k_4d.view(torch.int8), assumed_align=16)
        k_cute = k_cute.mark_layout_dynamic(leading_dim=3)
    ...
    if is_fp8_pv:
        v_cute = from_dlpack(v_4d.view(torch.int8), assumed_align=16)
        v_cute = v_cute.mark_layout_dynamic(leading_dim=3)
        v_cute.element_type = cutlass.Float8E4M3FN
    else:
        v_cute = from_dlpack(v_4d, assumed_align=16).mark_layout_dynamic(leading_dim=3)

    if is_fp8_out:
        o_cute = from_dlpack(o_4d.view(torch.int8), assumed_align=16)
        o_cute = o_cute.mark_layout_dynamic(leading_dim=3)
